...
/* Get data */
$sql = "select name$lang, slugvi, slugen, id, photo, regular_price, sale_price, discount, type from #_product where type='dich-vu' $where and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb,id desc";
...
?>
<?php if ($countItems) { ?>
<div class="grid_3 grid_2 padding__15 paddingg_25">
    <?php foreach ($items as $k => $v) { ?>
    <div class="productt">
        <a class="box-product text-decoration-none" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>">
            <div class="pic-product zoom_hinh hover_sang">
